...
function SetSelectedMods(s)
    if s then
        LOG("MultiplayerSave DEBUG: MOD LIST SET TO:")
        for uid,v in s do
            LOG("MultiplayerSave DEBUG: \t" .. _mod_cache[uid].name .. "\t(" .. uid .. ")")
        end
...
-- MODS: START PATCH
------------------------------------
LOG('MODS: Loading patch for mod functions')

-- sorts mods according to their before/after lists, otherwise alphabetically
-- returns true when first UID should load _after_ second
local function ModComp(first, second, mods)

	-- SpliFF: Bugfix. You need to compare both ways!
	if mods[first].before then
		for i,uid in mods[first].before do
			if mods[second].uid == uid then
				--LOG('MODS: '..mods[first].name..' before '..mods[second].name)
				return false
			end
		end
	end
	if mods[first].after then
		for i,uid in mods[first].after do
			if mods[second].uid == uid then
				--LOG('MODS: '..mods[first].name..' after '..mods[second].name)
				return true
			end
		end
	end
	if mods[second].before then
		for i,uid in mods[second].before do
			if mods[first].uid == uid then
				--LOG('MODS: '..mods[second].name..' before '..mods[first].name)
				return true
			end
		end
	end
	if mods[second].after then
		for i,uid in mods[second].after do
			if mods[first].uid == uid then
				--LOG('MODS: '..mods[second].name..' after '..mods[first].name)
				return false
			end
		end
	end

	--LOG('MODS: '..mods[second].name..' unrelated to '..mods[first].name)
	return first < second
end

-- Implementation of bubble sort which is slower but more accurate than quicksort
local function SortMods( uids, mods )
	LOG('MODS: bubble-sorting mods')
	local num_swaps = 0
	local uids_size = table.getn(uids)
	local i = 1
	while( i<=uids_size ) do 
		local j = 1
		while( j<=uids_size ) do 
			--LOG('MODS: compare '..uids[j]..' to '..uids[i])
			if( j~=i and ModComp(uids[j],uids[i],mods) ) then
				local tmp = uids[j]
				uids[j] = uids[i]
				uids[i] = tmp
...
local function GetActiveModsFiltered(filter, selected)
    ...
    -- get selected mods
    for uid,m in all_mods do
        if selected[uid] and filter(m) then
            table.insert(uids,uid)
	    mods[uid] = m
        end
    end
    -- get unselectable (hidden) dependencies
    for uid,m in mods do
	for _,depuid in m.requires do
		mods[depuid] = all_mods[depuid]
        end
    end
    -- return sorted result
    if not sorted_mods then
	sorted_mods = SortMods(uids, mods)
    end
    mods = {}
    for i,uid in ipairs(sorted_mods) do
	mods[i] = all_mods[uid]
    end
    return mods
end
